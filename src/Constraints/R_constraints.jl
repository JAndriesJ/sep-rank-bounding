module R_constraints
    ...
    function make_loc_cons_S₂(ρ,d,t,Lx)
        ...
        for b in keys(MB)
            ...
            g₁ = stρ*uc.idx2var(Lx, MB[b]) - xRterm # √(Tr(ρ))⋅L([x,y]ₜ₋₁[x,y]ₜ₋₁ᵀ) - L((∑xᵢ²)⋅[x,y]ₜ₋₁[x,y]ₜ₋₁ᵀ )
            g₂ = stρ*uc.idx2var(Lx, MB[b]) - yRterm # √(Tr(ρ))⋅L([x,y]ₜ₋₁[x,y]ₜ₋₁ᵀ) - L((∑yᵢ²)⋅[x,y]ₜ₋₁[x,y]ₜ₋₁ᵀ )
            loc_con[b,"x"] = g₁ ; loc_con[b,"y"] = g₂
        end
        return loc_con
    end
