...
for tiepe in ["ent","sep"] # ,
    t = 3
    csv_path = bounds_dir*"t=$t\\$tiepe\\$tiepe"*"t=$t.csv"

    df = read_n_proc_tools.proc_csv(csv_path)
    df_clean = read_n_proc_tools.proc_df_dict(df)

    save_path = bounds_dir*"t=$t\\$tiepe\\clean_$tiepe"*"t=$t.csv"
    CSV.write(save_path, df_clean, delim = "|")
end
